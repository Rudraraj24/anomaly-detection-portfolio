...
def main():
    ...
    try:
        ...
        print("Please run 'python scripts/train_models.py' first")
        return
    
    # Step 2: Generate new transactions to score
    print("\n[STEP 2] Generating new transactions...")
    generator = TransactionGenerator(seed=999)  # Different seed
    ...
    # Actual performance (since we have labels)
    actual_fraud = df_scored['is_fraud'].sum()
    detected_fraud = df_scored[df_scored['risk_level'].isin(['CRITICAL', 'HIGH'])]['is_fraud'].sum()
    
    print(f"\nActual Fraud in Batch: {actual_fraud}")
    print(f"Fraud Caught: {detected_fraud}")
    print(f"Detection Rate: {detected_fraud/actual_fraud*100 if actual_fraud > 0 else 0:.1f}%")
